...
vector<Sample> Utils::getTrainData(const string &filename) {
    ...
    for (size_t i = 0; i < buffer.size(); i += Config::INNODE + Config::OUTNODE) {
        ...
        for (size_t j = 0; j < Config::INNODE; ++j) trainSample.feature.push_back(buffer[i + j]);
        // Read in training sample 'label'
        for (size_t k = 0; k < Config::OUTNODE; ++k) trainSample.label.push_back(buffer[i + Config::INNODE + k]);
        trainDataSet.push_back(trainSample); // Add samples to the 'trainDataSet'
    }
    return trainDataSet;
}
...
vector<Sample> Utils::getTestData(const string &filename) {
    ...
    for (size_t i = 0; i < buffer.size(); i += Config::INNODE) {
        ...
        for (size_t j = 0; j < Config::INNODE; ++j) testSample.feature.push_back(buffer[i + j]);
        testDataSet.push_back(testSample); // Add samples to the 'testDataSet'
    }
    return testDataSet;
}
